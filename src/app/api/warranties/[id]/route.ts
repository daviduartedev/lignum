...
import { warrantyUpdateSchema } from "@/lib/zodSchemas";
...
export const GET = withRole(allStaffReadRoles, async (_req: NextRequest, ctx: RouteContext) => {
  ...
  const row = await prisma.warranty.findUnique({ where: { id: num } });
  if (!row) {
...
export const PUT = withRole(commercialWriteRoles, async (req: NextRequest, ctx: RouteContext) => {
  ...
  const parsed = warrantyUpdateSchema.safeParse(raw);
  if (!parsed.success) {
    return zodErrorResponse(parsed.error);
  }
  const d = parsed.data;
  const { startDate, endDate, ...rest } = d;
...
export const DELETE = withRole(commercialWriteRoles, async (_req: NextRequest, ctx: RouteContext) => {
  ...
  try {
    await prisma.warranty.delete({ where: { id: num } });
    return ok({ id: num });
  } catch {
    return fail("NOT_FOUND", 404);
  }
