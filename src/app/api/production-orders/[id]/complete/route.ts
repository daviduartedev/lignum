...
import { completeProductionOrder } from "@/lib/production/orderTransitions";
...
export const POST = withRole(productionWriteRoles, async (_req: NextRequest, ctx: RouteContext) => {
  ...
  try {
    const updated = await completeProductionOrder(internalId);
    return ok(updated);
  } catch (e) {
    const msg = e instanceof Error ? e.message : "";
    if (msg === "NOT_FOUND") return fail("NOT_FOUND", 404);
    if (msg === "INVALID_TRANSITION") {
      return fail("CONFLICT", 409, { message: "Não é possível concluir esta ordem de produção." });
    }
    throw e;
  }
});
